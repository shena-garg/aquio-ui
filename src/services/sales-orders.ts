...
export interface SORemainingItem {
  productId: string;
  variantId: string;
  remainingQuantity: number;
}

export interface SOReceiptProduct {
  productId: string;
  variantId: string;
...
export interface SOReceipt {
  _id: string;
  userId: string;
  deliveryDate: string;
  products: SOReceiptProduct[];
  notes: string;
  files: any[];
  createdAt: string;
}
...
export interface SalesOrder {
  ...
  commonUOM?: string;
  products?: SOProduct[];
  remainingItems?: SORemainingItem[];
  receipts?: SOReceipt[];
  notes?: string;
...
export interface SalesOrdersResponse {
  ...
  counts: SOStatusCounts;
}

/** Filters applied via the search bar — all optional */
export type SOActiveFilters = {
  poNumber?: string;
  referenceId?: string;
  supplierReferenceId?: string;
  status?: string;
...
export interface GetSalesOrdersParams extends SOActiveFilters {
  ...
  tabStatus?: SOFilterStatus;
}

export type CsvPattern = "basic" | "comprehensive";

export interface ExportSalesOrdersParams {
...
export const salesOrdersService = {
  ...
  exportList: (params: ExportSalesOrdersParams) =>
