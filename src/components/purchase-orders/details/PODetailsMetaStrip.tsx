...
import type { PurchaseOrder, OrderEntity } from "@/services/purchase-orders";
import { EntityInfoDrawer } from "./EntityInfoDrawer";

interface PODetailsMetaStripProps {
  order: PurchaseOrder;
}

export function PODetailsMetaStrip({ order }: PODetailsMetaStripProps) {
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="flex flex-col gap-1">
          ...
          <div className="flex items-center gap-1.5 text-[13px] font-medium text-[#111827]">
            {order.buyer?.name ?? "—"}
            {order.buyer && (
              <button onClick={() => setActiveEntity({ entity: order.buyer!, label: "Consignee" })}>
                <Info className="h-4 w-4 text-[#9ca3af] hover:text-[#6b7280] flex-shrink-0 transition-colors" />
              </button>
            )}
          </div>
