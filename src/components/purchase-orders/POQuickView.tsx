...
export function POQuickView({ po, onClose }: POQuickViewProps) {
  ...
  // Compute received qty per product from remainingItems
  function getReceivedQty(productId: string, variantId: string, ordered: number): number {
  ...
  const totalReceived = products.reduce((s, p) => {
    return s + getReceivedQty(p.product._id, p.variant._id, p.quantity.value);
  }, 0);
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[#e5e7eb]">
                    ...
                    <th className="text-right py-2 px-2 text-[11px] font-semibold text-[#6b7280] whitespace-nowrap">Recd Qty</th>
                ...
                <tbody>
                  {products.map((p, idx) => {
                    ...
                    const received = getReceivedQty(p.product._id, p.variant._id, ordered);
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-3 px-2 text-right text-[13px] text-[#111827] whitespace-nowrap">
                          {received.toLocaleString("en-IN", { minimumFractionDigits: 0, maximumFractionDigits: 3 })}
                          {p.quantity.postfix && (
                            <span className="text-[11px] text-[#9ca3af] ml-0.5">{p.quantity.postfix}</span>
                          )}
                        </td>
                ...
                <tfoot>
                  <tr className="border-t border-[#e5e7eb]">
                    ...
                    {/* Recd Qty column — Total Received */}
                    <td className="py-3 px-2 text-right whitespace-nowrap">
                      {hasUniformUOM && po.totalQuantity != null ? (
                        <span className="text-[13px] font-semibold text-[#111827]">
                          {totalReceived.toLocaleString("en-IN", { minimumFractionDigits: 0, maximumFractionDigits: 3 })}
                          <span className="text-[11px] text-[#9ca3af] font-normal ml-0.5">{commonUOM}</span>
                        </span>
                      ) : null}
                    </td>
