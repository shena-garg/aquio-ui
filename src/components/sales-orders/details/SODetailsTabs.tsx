...
export function SODetailsTabs({ order }: SODetailsTabsProps) {
  ...
  return (
    <div className="flex flex-col">
      {/* Tab bar */}
      <div className="flex overflow-x-auto border-b border-[#e5e7eb] bg-white px-4 sm:px-8 sticky top-[56px] lg:top-0 z-10">
        {tabs.map((tab) => {
    ...
    force_closed: {
      ...
      icon: (
        <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
          <path d="M2 2l6 6M8 2l-6 6" stroke="#ea580c" strokeWidth="1.2" strokeLinecap="round" />
        </svg>
      ),
    },
  } as const;
...
function ShipmentSummaryCard({
  ...
  async function handleForceCloseConfirm() {
    ...
    try {
      const orderId = order.id ?? order._id;
      if (forceCloseModal.mode === "close") {
        await salesOrdersService.forceCloseMultiple(orderId, [{ productId: forceCloseModal.productId, variantId: forceCloseModal.variantId }]);
        toast.success("Product force closed successfully");
      } else {
        await salesOrdersService.undoForceClose(orderId, forceCloseModal.productId, forceCloseModal.variantId);
        toast.success("Force close undone successfully");
      }
              ...
              <table style={{ width: '640px', tableLayout: 'fixed' }}>
                ...
                <tbody ref={leftTbodyRef} className="[&>tr:last-child>td]:border-b-0">
                  {productList.map(({ productId, variantId }, idx) => {
                    ...
                    const status = getShipmentStatus(totalShipped, ordered, remainingItem?.status);
                        ...
                        >
                          {isRightForceClosed ? (
                            ...
                            <div className="flex items-center">
                              <QuantityCell value={remaining} uom={uom} />
                              {canForceClose && remaining > 0 && (
                                <button
                                  title="Force Close"
                                  onClick={() => {
                                    const { name, variant } = getProductName(productId, products);
                                    setForceCloseModal({
                                      open: true, mode: "close",
                                      productId, variantId,
                                      productName: name, variantName: variant,
                                    });
                                  }}
                                  className="text-[#9ca3af] hover:text-[#ea580c] transition-colors flex-shrink-0 ml-1"
                                >
                                  <XIcon size={12} />
                                </button>
                              )}
            ...
            <table className="w-full" style={{ minWidth: `${280 + receipts.length * 100 + 90}px` }}>
              ...
              <tbody className="[&>tr:last-child>td]:border-b-0">
                {productList.map(({ productId, variantId }) => {
                  ...
                  const mobileStatus = getShipmentStatus(totalShippedQty, ordered, mobileRemainingItem?.status);
    ...
    <>
      ...
      <div className="sm:hidden px-4 pt-3 pb-4 space-y-2">
        {products.map((product, idx) => {
          ...
          const isForceClosed = remaining?.status === "forcefully closed";
          const shippedBarColor = isForceClosed
            ? "bg-[#9ca3af]"
            : isNotShipped
              ? "bg-[#d1d5db]"
              : "bg-[#10b981]";
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      {canForceClose && remainingQty > 0 && (
                        <button
                          ...
                          className="text-[#9ca3af] hover:text-[#ea580c] transition-colors flex-shrink-0"
                        >
                          <XIcon size={12} />
                        </button>
                      )}
