...
export default function CreatePurchaseOrderPage() {
  ...
  async function handleSubmit(status: "issued" | "draft") {
    ...
    if (!supplierCompany || !supplierLocation) {
      setSubmitError("Please select a supplier and location.");
      return;
    }
    if (!consigneeCompany || !consigneeLocation) {
      setSubmitError("Please select a consignee and location.");
      return;
    }
    if (!buyerCompany || !buyerLocation) {
      setSubmitError("Please select a buyer and location.");
      return;
    }
    if (!settings?.generatePOAutomatically && !poNumber.trim()) {
    ...
    if (completeRows.length === 0) {
      setSubmitError(
        "At least one product with quantity and price is required."
      );
      return;
    }
    ...
    if (hasIncomplete) {
      setSubmitError(
        "Some product rows are incomplete. Please fill in all fields or remove them."
      );
      return;
    }
    ...
    try {
      ...
      const payload: CreatePOPayload = {
        ...
        orderType: "purchase",
        status,
        poNumber: settings?.generatePOAutomatically ? "" : poNumber,
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* ── Error banner ────────────────────────────────────────────────── */}
      {submitError && (
        <div className="flex items-center justify-between bg-[#fef2f2] border-b border-[#fecaca] px-6 py-2.5">
            ...
            <div className="grid grid-cols-4 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full h-9 border border-[#e5e7eb] rounded-[6px] px-3 text-[13px] text-[#111827] outline-none focus:ring-2 focus:ring-[#0d9488] focus:border-[#0d9488] bg-white"
                >
                    ...
                    <tbody>
                      {productRows.map((row) => {
                        ...
                        return (
                          <tr
                            key={row.id}
                            className="border-b border-[#f3f4f6] hover:bg-[#fafafa] transition-colors divide-x divide-[#e5e7eb]"
                          >
                            {/* Product */}
