...
- **Home view — drill-down navigation** — The home view shows a clean 6-item menu (Order Health, Follow-up Needed, Reorder Suggestions, Supplier Budgets, Draft Order, Ask Aqira). Each row shows a count badge or status hint. Tapping any row replaces the panel with that section's full content and a Back link. All other sections are hidden while focused. Ask result and draft prompt persist across navigation.
- **Order Health** — Split by Purchase Orders and Sales Orders. Shows overdue count, draft count, and due-this-week count in a table with two columns (PO / SO). Each number is a clickable link to the filtered order list. Backed by 6 parallel `countDocuments` queries on `GET /aqira/home`.
- **Follow-up Needed** — Lists confirmed purchase orders that are past their delivery date with no receipt activity yet. Shows supplier, days overdue, and a "Copy Message" button that generates a ready-to-send follow-up template. Backed by `GET /aqira/home`.
- **Reorder Suggestions** — Surfaces products that are overdue for reorder based on historical average order interval (min 2 prior orders, min 7-day interval). Shows days overdue, last supplier, and a "Draft Reorder" button that pre-fills the PO create form. Backed by a MongoDB aggregation on `GET /aqira/home`.
- **Supplier Budgets** — Set a monthly spend limit per supplier. Progress bar shows current month spend vs limit. Turns amber at 80%, red when over. Add/delete from the panel. `aqirabudgets` collection. CRUD via `GET/POST/DELETE /aqira/budgets`.
- **Budget Status on forms** — On PO/SO create and edit pages, if the selected supplier has a budget set, a compact budget bar appears below Supplier Comparison showing current month spend vs limit with colour-coded warning.
...
- **Context awareness** — Form pages show Price Alerts + Supplier Comparison + Budget Status + Ask. Order detail pages show Order Summary + Draft/Ask tabs. All other pages show the drill-down home view.
...
| Aqira | `POST /aqira/draft-order`, `POST /aqira/ask`, `GET /aqira/home`, `GET/POST /aqira/budgets`, `DELETE /aqira/budgets/:entityId`, `GET /aqira/budget-status` |
